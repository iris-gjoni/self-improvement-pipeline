...
Usage:
    python runner.py "Add user authentication with JWT tokens" --language python
...
class PipelineRunner:
    ...
    def run(self) -> dict:
        ...
        for i, step in enumerate(steps, 1):
            ...
            # Skip if already completed on a resume
    ...
    def _run_tdd_red(self, step, system_prompt, model, user_message, max_tokens) -> dict:
        ...
        # Install deps before running tests
        ...
        if total == 0:
            console.print("  [yellow]Warning: No tests were found in the workspace.[/yellow]")
        elif test_result["passed"] and failed == 0:
            console.print(
                "  [yellow]Warning: All tests are passing — TDD Red expects them to FAIL.[/yellow]"
            )
        else:
    ...
    def _run_tdd_green(self, step, system_prompt, model, user_message, max_tokens) -> dict:
        ...
        install_result = self.executor.install_dependencies()
        if not install_result["success"]:
            console.print(f"  [yellow]Dependency install warning[/yellow]")
        ...
        for attempt in range(1, max_attempts + 1):
            ...
            if attempt == 1:
                ...
                retry_msg = (
                    ...
                    f"**Current test results:** {test_result['passed_count']} passed, "
                    f"{test_result['failed_count']} failed, {test_result['error_count']} errors\n\n"
                    f"```\n{test_result['formatted_output']}\n```\n\n"
                    f"Focus on fixing the remaining failures. Remember:\n"
                    f"- Do NOT modify any test files\n"
                    f"- Do NOT delete or skip tests\n"
            ...
            if type_result.get("errors"):
                count = len(type_result["errors"])
                console.print(f"  [yellow]Type errors: {count}[/yellow]")
            ...
            total = test_result["total"]
            console.print(
                f"  [dim]Tests: {passed}/{total} passed, {failed} failed, {errors} errors[/dim]"
            )
            ...
            if all_passing:
                console.print(f"  [green]✓ All {total} tests passing![/green]")
                return {
                    "status": "success",
                    "attempts": attempt,
                    "test_result": test_result,
        ...
        console.print(
            f"  [red]Final state: {test_result['passed_count']} passed, "
            f"{test_result['failed_count']} failed[/red]"
        )
        return {
            "status": "failed",
    ...
    def _run_integration(self, step, system_prompt, model, user_message, max_tokens) -> dict:
        ...
        passed = integration_result["passed_count"]
        failed = integration_result["failed_count"]
        total = integration_result["total"]
        console.print(
            f"  Integration tests: {passed}/{total} passed, {failed} failed"
        )

        status = "success" if integration_result["passed"] or failed == 0 else "partial"
        return {
            "status": status,
            "output": output,
    ...
    def _run_postmortem_step(self, step, system_prompt, model, user_message, max_tokens) -> dict:
        ...
        console.print(f"  [green]✓ Post-mortem complete: {n} proposal(s) generated[/green]")
        console.print(f"  [dim]Review with: python apply_proposal.py {proposal_path}[/dim]")
    ...
    def _init_workspace(self) -> None:
        ...
        init_files = lang_config.get("init_files", {})
    ...
    def _print_summary(self) -> None:
        table = Table(title=f"Pipeline Summary — {self.run_id}", border_style="blue")
        table.add_column("Step", style="cyan", min_width=30)
        table.add_column("Status", style="bold", min_width=10)
        table.add_column("Duration", style="dim", min_width=10)
        ...
        for step in self.pipeline["steps"]:
            ...
            if step_id in ("tdd_green", "tdd_red") and "test_result" in result:
                ...
                notes = f"{tr.get('passed_count', 0)}/{tr.get('total', 0)} tests passed"
        ...
        console.print("\n")
        console.print(table)
        console.print(f"\n[dim]Full run artifacts: {self.run_dir}[/dim]")
...
def main():
    ...
    parser.add_argument(
        "feature_request",
        nargs="?",
        help="Feature request or description of what to build",
    )
    parser.add_argument(
        "--language",
        "-l",
    ...
    parser.add_argument(
        "--resume",
        metavar="RUN_ID",
        help="Resume an interrupted pipeline run by its ID",
    )
    parser.add_argument(
        "--skip-postmortem",
        action="store_true",
...
def _list_runs() -> None:
    ...
    table.add_column("Feature Request", style="dim", max_width=60)
    ...
    for run_dir in sorted(runs_dir.iterdir(), reverse=True):
        ...
        try:
            ...
            color = {"completed": "green", "interrupted": "yellow", "running": "blue"}.get(
                status, "dim"
            )
            table.add_row(
                meta.get("id", run_dir.name),
                f"[{color}]{status}[/{color}]",
                (meta.get("feature_request", "")[:60]),
                meta.get("language", "?"),
            )
        except Exception:
